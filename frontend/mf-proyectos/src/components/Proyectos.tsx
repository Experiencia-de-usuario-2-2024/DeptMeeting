...
const Proyectos: React.FC<{ periodo?: { _id: string, name:string,  commissions: string[], meetings: string[]}}> = ({ periodo }) => {
    ...
    const [periodoElegido, setPeriodoElegido] = React.useState<{ _id: string, name:string,  commissions: string[], meetings: string[]}>({});
    ...
    useEffect(() => {
        ...
        if (periodo) {
            ...
            if (periodoGuardado) {
                setPeriodoElegido(JSON.parse(periodoGuardado));
            }
        }
        ...
        setProyectosUser(periodo?.commissions || []);

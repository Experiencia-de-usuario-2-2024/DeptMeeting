...
export class NotificationController {
  ...
  async create(
    @Body() meetingDTO: NotificationDTO,
  ): Promise<Observable<INotification>> {
    return this._ClientProxyNotification.send(
      NotificationMSG.CREATE,
      meetingDTO,
    );
  }
  ...
  async findAll(): Promise<Observable<INotification[]>> {
    return this._ClientProxyNotification.send(NotificationMSG.FIND_ALL, '');
  }
  ...
  async findOne(@Param('id') id: string) {
    return this._ClientProxyNotification.send(NotificationMSG.FIND_ONE, id);
  }
  ...
  async update(
    ...
    return this._ClientProxyNotification.send(NotificationMSG.UPDATE, {
      id,
      notificationDTO,
    });
  }

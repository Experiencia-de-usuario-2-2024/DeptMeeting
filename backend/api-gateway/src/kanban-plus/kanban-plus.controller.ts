...
constructor(private readonly clientProxy: ClientProxyMeetflow) { }
  ...
  async addProject(@Body() kanbanPlus: any, @Req() req: any) {
    ...
    return this._clientProxyKanbanPluls.send('CREAR_KANBAN_PLUS', kanbanPlus);
  }

  @Get('/get/:id')
  @ApiOperation({ summary: 'obtener kanban plus por id' })
  async findOne(@Param('id') id: string) {
    return this._clientProxyKanbanPluls.send('BUSCAR_UN_KANBAN_PLUS', id);
  }

  @Get('/get-all')
  @ApiOperation({ summary: 'obtener todos los kanban plus' })
  async findAll() {
    return this._clientProxyKanbanPluls.send('BUSCAR_TODOS_KANBAN_PLUS', '');
  }
  ...
  async update(
    ...
    return this._clientProxyKanbanPluls.send("ACTUALIZAR_KANBAN_PLUS", { id, kanbanPlus });
  }

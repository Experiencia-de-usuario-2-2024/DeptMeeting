...
export class DoodleController {
    ...
    private _clientProxyDoodle = this.clientProxy.clientProxyDoodle();
      ...
      async addProject(@Body() doodle: any, @Req() req: any) {
        ...
        return await this._clientProxyDoodle.send('CREAR_DOODLE', doodle);
      }
    
      @Get('/get/:id')
      @ApiOperation({ summary: 'obtener doodle por id' })
      async findOne(@Param('id') id: string) {
        return await this._clientProxyDoodle.send('BUSCAR_UN_DOODLE', id);
      }
    
      @Get('/get-all')
      @ApiOperation({ summary: 'obtener todos los doodle' })
      async findAll() {
        return await this._clientProxyDoodle.send('BUSCAR_TODOS_DOODLE', '');
      }
      ...
      async update(
        ...
        return await this._clientProxyDoodle.send("ACTUALIZAR_DOODLE", { id, doodle });
      }

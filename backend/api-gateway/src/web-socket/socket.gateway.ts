  ...
  */
  ...
  handleDisconnect(client: any) {
    ...
    Object.keys(this.connectedUsers).forEach((room) => {
      this.connectedUsers[room] = this.connectedUsers[room].filter(
  ...
  handleIncomingMessage(
    ...
    this.server
      .to('room_' + room)
      .emit('new_message', { user: user, message: message });
  }

  // EVENTO DE RECIBIR MENSAJE DE GUARDAR
  @SubscribeMessage('event_save')
  handleIncomingMessage2(client: Socket, payload: { room: string; user: any }) {
  ...
  handleIncomingMessage3(client: Socket, payload: { room: string; user: any }) {
    const { room, user } = payload;
  ...
  handleIncomingMessage4(client: Socket, payload: { room: string; user: any }) {
    const { room, user } = payload;
  ...
  handleIncomingMessage5(client: Socket, payload: { room: string; user: any }) {
    const { room, user } = payload;
  ...
  handleIncomingMessage7(client: Socket, payload: { room: string; user: any }) {
    const { room, user } = payload;
  ...
  handleIncomingMessage8(client: Socket, payload: { room: string; user: any }) {
    const { room, user } = payload;
  ...
  handleRoomLeave(client: Socket, payload2: { room: string; user: any }) {
    ...
    if (this.connectedUsers[payload2.room]) {
      this.connectedUsers[payload2.room] = this.connectedUsers[
        payload2.room
  ...
  handleJoinRoom(client: Socket, payload: { room: string; user: any }) {
    ...
    client.emit(
      'connected_users',
      this.connectedUsers[payload.room].map((entry) => entry.user),
    );
  ...
  handlePreMeeting(client: Socket, payload: { room: string; user: any }) {
    console.log(
      '[socket] usuario: ' +
        payload.user.name +
        ' ha cambiado actualizado el estado de la reunión',
    );
  ...
  handleAddElement(
    client: Socket,
    payload: { room: string; user: any; element: any },
  ) {
  ...
  handleUpdateElement(
    ...
    console.log(
      '[socket] usuario: ' + payload.user.name + ' ha actualizado un elemento',
    );
    this.server.to('room_' + payload.room).emit('element_updated', {
  ...
  handleDeleteElement(
    ...
    console.log(
      '[socket] usuario: ' + payload.user.name + ' ha eliminado un elemento',
    );
    this.server.to('room_' + payload.room).emit('element_deleted', {
      element: payload.element,
      user: payload.user,
    }); // Los clientes escuchan este evento
  }
  ...
  handleBlockElement(
    ...
    console.log(
      '[socket] usuario: ' + payload.user.name + ' ha bloqueado un elemento',
    );
    this.server.to('room_' + payload.room).emit('element_blocked', {
      element: payload.element,
      user: payload.user,
    });
  }
  ...
  handleUnblockElement(
    client: Socket,
    payload: { room: string; user: any; element: any },
  ) {
    console.log(
      '[socket] usuario: ' + payload.user.name + ' ha desbloqueado un elemento',
    );
    this.server.to('room_' + payload.room).emit('element_unblocked', {
      element: payload.element,
      user: payload.user,
    });
  }

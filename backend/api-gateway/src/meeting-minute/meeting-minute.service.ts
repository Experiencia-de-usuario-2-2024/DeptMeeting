...
export class MeetingMinuteService {
  ...
  async sendNotificationExternal(
    ...
    console.log(
      '[MEETING MINUTE SERVICE DESDE API] ESTOY ENVIANDO LO SIGUIENTE COMO EVENTO AL INVITADO EXTERNO: ',
      meetingMinuteDTO,
      user,
    );

    return this.eventEmitter2.emit(
      'meetingMinute.inviteExternal',
      meetingMinuteDTO,
      user,
    );
  }
  ...
  async sendNotification(meetingMinuteDTO: any, user: any): Promise<any> {
    console.log(
      '[MEETING MINUTE SERVICE DESDE API] ESTOY ENVIANDO LO SIGUIENTE COMO EVENTO: ',
      meetingMinuteDTO,
      user,
    );
  ...
  async sendNotificationRemember(remember: any, user: any): Promise<any> {
    ...
    return this.eventEmitter2.emit('meetingMinute.rembember', remember, user);
  }
  ...
  eventActivationTime(remember: any, miliseg: number, user: any) {
    ...
    setTimeout(async () => {
      this.eventEmitter2.emit('meetingMinute.rembemberTask', remember, user);
    }, miliseg);
    return true;
  }
}

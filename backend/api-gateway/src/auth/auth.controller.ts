...
export class AuthController {
  ...
  async signIn(@Body() loginDto: any) {
    ...
    const isExist = await firstValueFrom(
      this._clientProxyUser.send('VALID_USER', loginDto),
    );
    if (isExist) {
      ...
      throw new UnprocessableEntityException(
        'No existe el usuario especificado o la contraseña es incorrecta.',
      );
    }
  }
  ...
  async resetPass(@Param('email') email: string): Promise<Observable<any>> {
    const user = await firstValueFrom(
      this._clientProxyUser.send('RESET_PASS', email),
    );
    if (user) {
      console.log(
        'ENTRE AL IF PARA CAMBIAR LA CONTRASEÑA DEL USUARIO: ' + user.email,
      );
      await firstValueFrom(
        this._clientProxyNotifications.send('SEND_PASS', user),
      );
      return user;
    } else {
      throw new UnprocessableEntityException(
...
*/
  ...
  async notifyNewUser(@Body() newUser: any): Promise<Observable<any>> {
    return await firstValueFrom(
      this._clientProxyNotifications.send('SEND_INVITED_NEW_USER', newUser),
    );
  }
}

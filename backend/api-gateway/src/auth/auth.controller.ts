...
export class AuthController {
  ...
  async signIn(@Body() loginDto: any) {
    ...
    const isExist = await this._clientProxyUser
      .send('VALID_USER', loginDto)
      .toPromise();
    if (isExist) {
  ...
  async signUp(@Body() userDTO: UserDTO) {
    ...
    if (isExist) {
      ...
      throw new UnprocessableEntityException(
        'Usuario ya registrado, favor de iniciar sesión:w.',
      );
    }
  }
...
*/
  ...
  async notifyNewUser(@Body() newUser: any): Promise<Observable<any>> {
    return await this._clientProxyNotifications
      .send('SEND_INVITED_NEW_USER', newUser)
      .toPromise();
  }
}
